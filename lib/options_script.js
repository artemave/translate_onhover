function save_options() {
  ...
  Options.word_alt_only($('#word_alt_only:checked').val() ? 1 : 0);
  Options.selection_alt_only($('#selection_alt_only:checked').val() ? 1 : 0);
  ...
  Options.popup_show_trigger($('#word_alt_only_key').val())
...
function fill_source_lang() {
  ...
  $('#source_lang').change(function() {
      ...
      if (source_lang.match(/autodetected_from_/)) {
        $('#reverse_lang').attr('disabled', false);
      }
      else {
        $('#reverse_lang').attr('disabled', true).val($('#source_lang').val());
      }
  });
...
function populate_popup_show_trigger() {
  ...
  modifierKeys.values().uniq().forEach(function(key) {
    $('#word_alt_only_key, #selection_alt_only_key').each(function() {
      $(this).append($('<option>', {value: key}).text(key).prop('selected', saved_popup_show_trigger == key))
    })
  })

  $('#word_alt_only_key, #selection_alt_only_key').change(function() {
    $('#word_alt_only_key, #selection_alt_only_key').val(this.value)
  })
}
...
$(function() {
    ...
    $('#translate_by').val(Options.translate_by()).change(function() {
        if ($(this).val() == 'point' && !$('#word_alt_only').attr('checked')) {
          $('#delay').attr('disabled', false).parent().removeClass('disabled');
        }
        else {
          $('#delay').attr('disabled', true).parent().addClass('disabled');
    ...
    $('#word_alt_only').attr('checked', Options.word_alt_only() ? true : false).click(function() {
    ...
    $('#selection_alt_only').attr('checked', Options.selection_alt_only() ? true : false);
